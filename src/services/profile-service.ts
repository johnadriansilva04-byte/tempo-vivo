...
function getUserId(): string {
  // Se não tiver Supabase configurado, usa o singleton local
  if (!isSupabaseConfigured) {
    return "00000000-0000-0000-0000-000000000001";
  }
  
  // Se tiver cache, usa o cache
  if (cachedUserId) {
    return cachedUserId;
  }
  
  // Se tiver Supabase, tenta pegar o user_id da sessão atual (síncrono)
  try {
    ...
    // Fallback para singleton se der erro
  }
  
  return "00000000-0000-0000-0000-000000000001";
}

// Função para atualizar o cache quando o auth mudar
export function setUserIdCache(userId: string | null) {
  cachedUserId = userId;
}
...
export async function getProjects(): Promise<Project[]> {
  if (isSupabaseConfigured) {
    try {
      const db = await requireSupabase();
      const userId = getUserId();
      const { data, error } = await db.from("projects").select("*").eq("user_id", userId);
      if (error) throw error;
...
export async function upsertProject(project: Project): Promise<void> {
  if (isSupabaseConfigured) {
    ...
    const { error } = await db
      .from("projects")
      .upsert({ id: project.name, user_id: userId, ...project } as never);
    if (error) throw error;
    return;
  }
...
export async function createMilestone(item: Milestone): Promise<void> {
  if (isSupabaseConfigured) {
    const db = await requireSupabase();
    const userId = getUserId();
    const { error } = await db.from("milestones").insert({ user_id: userId, ...item } as never);
    if (error) throw error;
    return;
  }
